...
module.exports = async message => {
  ...
  let muteRol = client.ayarlar.MUTED;
  ...
  if (check && !message.member.roles.cache.get(muteRol)) {
	...
	let data = await Stat.findOne({userID: message.author.id, guildID: message.guild.id}) || {messageLevel: 0};
	 ...
	 if (siradakilevel <= data.messageXP) {
	 	...
	 	const background = await loadImage("https://cdn.discordapp.com/attachments/852824522184065035/973239387385102376/meriazlevel.png");            
		...
		image.printText(`${data.messageLevel+1}`, 220, 77,350)
	 	...
	 	image.save()
	 	const attachment = new MessageAttachment(image.toBuffer(), 'level.png');
