...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    message.channel.send(new MessageEmbed().setColor("65D1F4").setFooter(`${message.guild.name} Sunucusuna ait level sıralaması yukarıda belirtildiği gibidir.`).setTitle("Top 10 Ses / Mesaj Level Sıralaması").setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setDescription(`${göster.join("\n")}`))
